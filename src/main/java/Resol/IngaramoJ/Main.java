    ...
    private static void menuEmpleados(Scanner sc, EmpleadoDAO empDAO) {
        ...
        try {
            switch (op) {
                ...
                case "6" -> {
                    // Si no tenés columnas/SQL para esto, no falla:
                    try {
                        empDAO.listarConAntiguedadYBono()
                ...
                case "7" -> {
                    // Idem: se intenta y, si no existe/BD no tiene fechas, avisa:
                    try {
                        empDAO.empleadosCumpleMes()
                                .forEach(System.out::println);
                    } catch (Throwable t) {
                        System.out.println("No disponible (faltan columnas de fecha en la BD o método en DAO).");
                    }
